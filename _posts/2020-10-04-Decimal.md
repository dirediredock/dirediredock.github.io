...
To use this function, add the following string at the end of a new `figure` while the function code is in a separate `.m` script within the same folder (as a standalone function in the working directory), or at the very end of the same `.m` script.

```{}
decimal_10=datacursormode();set(decimal_10,'DecimalGUI',@DecimalGUI,'Enable','on');
```
